...
/* Find a texture by name. Linear: n_tex is a uint16_t in the header,
 * and this runs when a cover arrives rather than per frame, so a
 * sorted table would be machinery for nothing. */
 ...
 * string.h only, and the digit loop bounds itself -- row is a uint16_t,
 * so five digits and a terminator always fit `digits`, and every write
 * to `out` is guarded against `cap`. */
