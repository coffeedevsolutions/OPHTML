...
**Measured:** Bench S12, SCPH-50000, HW #262, oplenv.elf at 5ea02ae. gs 0.91 to 0.93 ms against a 16.683 ms NTSC field -- 5.5%. With ee at 2.41 ms the two together are 3.34 ms, 20.0% of a field, leaving 13.34 ms unused; the worst photographed frame, ee peak 3.66 plus gs 0.93, is 4.59 ms and 27.5%.
...
THE ARM IS WHY THIS IS A MEASUREMENT. gs read 0.92 on the plain build and 3.21 on oplenv-fill.elf, so the CSR FINISH bit is not latched. It moved by 2.29 ms for eight full-screen alpha-blended sprites -- 2,293,760 blended pixels, which is 1.00 Gpix/s, sitting where a GS's alpha-blended fill rate is expected to sit. A latched instrument reads 0.00 forever; a broken one would not have landed on the hardware's own number.
...
WHAT IS STILL SMEARED: gs has no peak-hold, and the scroll frame pushes 28,224 bytes through the same chain, so the figure averages away the frames most likely to spike it. Comparing the fill build's up0 and up28224 windows hints at a few tenths of a millisecond per upload frame, but that is 0.01 ms of mean difference against a 0.01 ms print resolution, which is not a measurement. A gs peak-hold would settle it and costs one build.
