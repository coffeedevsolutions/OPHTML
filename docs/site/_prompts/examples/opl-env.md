# Page: examples/opl-env (opl-env)

Page type: `example`. Section order: `61`. Wave: `3`. Model tier: `sonnet`.
...
What the `opl-env` example demonstrates, how to build and check it, the numbers its blob carries, and which files to read first. Covers: six screens, two themes, 137 slots, ten streamed slots, one overlay; the driver in window.h; figures checked by tools/check-example-figures.py.
...
- examples/opl-env/ - every file: ps2ui.json, build.sh, ui/*, screenshots/, README.md, check.py
- tools/check-blobs.sh - the flags CI checks this blob with
- tools/check-example-figures.py - for opl-env
...
1. `./examples/opl-env/build.sh` builds, tests and refreshes screenshots -> run it and paste the last lines
2. The blob's numbers (screens, commands, textures, slots, arena) -> run `ps2ui-check examples/opl-env/build/ui.uib` and paste the trailer and arena note
...
5. Committed screenshots match the renderer -> `git diff --exit-code examples/opl-env/screenshots`
6. The figures in README.md match the blob -> run `python3 tools/check-example-figures.py`
...
- assets/examples/opl-env/<screen>[-theme-N].png <- `preview.render` per screen (per theme where n_theme > 1) // <screen>, theme N
- assets/examples/opl-env/states.png <- `preview.montage` // every focus state
...
- `example.opl-env.numbers` - the trailer values
- `example.opl-env.mechanisms` - the table
...
- [ ] `_facts/examples/opl-env.md` exists with every fact id listed under "Facts to emit", each with a status.
- [ ] Every command shown on the page was run in this session; every output block is pasted from that run.
- [ ] Every screenshot listed exists under `assets/examples/opl-env/`, is registered in `assets/assets.json` with its command, and has alt text.
- [ ] Frontmatter present: `id: examples/opl-env`, `title`, `description`, `section`, `order: 61`, `version: 0.6.0`, `sources` (every path opened).
