# Page: project/glossary (Glossary)

Page type: `project`. Section order: `74`. Wave: `3`. Model tier: `sonnet`.
...
- the facts files under Read first
- runtime/ps2ui.h comments and docs/format-uib.md for GS terms
...
1. Each definition matches the facts file it cites -> list ids
...
One H3 per term, alphabetical: arena · atlas · baker · blob · CLUT · CSM1 · command list · composite · display aspect · focus node · GS · gsKit · IR · kern pair · layout stage · list window · modulate domain · montage · nine-patch · PAR · pen · previewer · PSMT8 · PSMCT32 · role · scissor · screen · slot (text) · slot (texture) · tint table · title-safe · .uib · VRAM budget.
...
- [ ] `_facts/project/glossary.md` exists with every fact id listed under "Facts to emit", each with a status.
- [ ] Every command shown on the page was run in this session; every output block is pasted from that run.
- [ ] Every screenshot listed exists under `assets/project/glossary/`, is registered in `assets/assets.json` with its command, and has alt text.
- [ ] Frontmatter present: `id: project/glossary`, `title`, `description`, `section`, `order: 74`, `version: 0.6.0`, `sources` (every path opened).
