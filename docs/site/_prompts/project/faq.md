# Page: project/faq (FAQ)

Page type: `project`. Section order: `73`. Wave: `3`. Model tier: `sonnet`.
...
Fifteen to twenty-five questions the code answers, each in two to four sentences with one link.

## Read first

1. `docs/site/ARCHITECTURE.md`, all of it. The rules below are copied from it and are binding.
2. Parent facts files, before opening any source: `_facts/authoring/css.md`, `_facts/authoring/text-and-fonts.md`, `_facts/getting-started/installation.md`, `_facts/runtime/frame-loop.md`, `_facts/runtime/errors-and-constants.md`, `_facts/runtime/streaming-art.md`, `_facts/cli/previewer.md`, `_facts/authoring/screens-and-overlays.md`, `_facts/runtime/moving-and-hiding.md`, `_facts/authoring/vram-budget.md`, `_facts/authoring/dynamic-text.md`. Reuse their fact ids; do not restate a parent fact differently. If one is wrong, write a `## disputes` section and stop.
...
- docs/findings.md - for the two hardware faults (alpha inverted; clear not paying fill rate), one sentence each
...
1. Every answer cites a fact id -> list them in the facts file; no new verification
...
- `faq.questions` - the list

## Out of scope

- anything needing more than four sentences -> the linked page

## Done when

- [ ] Environment block ran clean.
- [ ] `_facts/project/faq.md` exists with every fact id listed under "Facts to emit", each with a status.
- [ ] Every command shown on the page was run in this session; every output block is pasted from that run.
- [ ] Every screenshot listed exists under `assets/project/faq/`, is registered in `assets/assets.json` with its command, and has alt text.
- [ ] Frontmatter present: `id: project/faq`, `title`, `description`, `section`, `order: 73`, `version: 0.6.0`, `sources` (every path opened).
