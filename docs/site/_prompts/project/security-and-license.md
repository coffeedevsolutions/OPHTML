# Page: project/security-and-license (Security and license)

Page type: `project`. Section order: `72`. Wave: `4`. Model tier: `sonnet`.
...
How to report, what the runtime and tools do with untrusted input, CI's posture, and the licences of the code and vendored assets.
...
- SECURITY.md
- LICENSE
- fonts/vendor/LICENSE.txt
- runtime/vendor/README.md
- packages/baker/pyproject.toml license field
- packages/layout/package.json license field
...
1. Report privately via the Security tab -> restate
2. The runtime validates every offset and rejects unknown versions; blobs from strangers are still untrusted input -> restate; link errors page
3. CI is read-only and hosted-only -> cite ci.yml permissions
4. MIT for the packages; DejaVu under its own licence; gsKit headers vendored under theirs -> cite files
...
Project skeleton: Reporting · Scope · CI · Licences (table: component · licence · file).
...
- runtime/errors-and-constants - validation
- project/contributing - runners
...
- [ ] `_facts/project/security-and-license.md` exists with every fact id listed under "Facts to emit", each with a status.
- [ ] Every command shown on the page was run in this session; every output block is pasted from that run.
- [ ] Every screenshot listed exists under `assets/project/security-and-license/`, is registered in `assets/assets.json` with its command, and has alt text.
- [ ] Frontmatter present: `id: project/security-and-license`, `title`, `description`, `section`, `order: 72`, `version: 0.6.0`, `sources` (every path opened).
