# Page: project/internals (Internals)

Page type: `project`. Section order: `75`. Wave: `4`. Model tier: `sonnet`.
...
One paragraph per repository document under docs/ saying what it is, who should read it and where it lives; the seven testing-failure shapes by name; the findings graph in two sentences. Nothing is rewritten.
...
- docs/architecture.md, docs/PLAN.md, docs/method.md, docs/findings.md, docs/findings.yaml, docs/bench-phase1.md, docs/bench-phase2.md, docs/bench-runbook.md, docs/bringup.md, docs/design-bringup-3-to-9.md, docs/design-p3b-theming.md, docs/design-v6-resource-model.md, docs/releasing.md - headings and first paragraphs only
- tools/check-findings.py docstring
...
1. Each paragraph names the file and its first heading accurately -> grep the H1 of each
2. The seven shapes -> the H3 names under 'The seven shapes' in method.md
...
Project skeleton: Decisions (architecture, PLAN) · Method (method, findings) · Hardware (bringup, bench-*, design-bringup) · Designs (v6 resource model, theming) · Releasing. Each entry: title as link, two or three sentences.
...
- reference/uib-format - pledge
- runtime/first-boot - bringup
- authoring/theming - design doc
- runtime/frame-loop - resource model
...
- Anything not listed under Purpose.

## Done when

- [ ] Environment block ran clean.
- [ ] `_facts/project/internals.md` exists with every fact id listed under "Facts to emit", each with a status.
- [ ] Every command shown on the page was run in this session; every output block is pasted from that run.
- [ ] Every screenshot listed exists under `assets/project/internals/`, is registered in `assets/assets.json` with its command, and has alt text.
- [ ] Frontmatter present: `id: project/internals`, `title`, `description`, `section`, `order: 75`, `version: 0.6.0`, `sources` (every path opened).
