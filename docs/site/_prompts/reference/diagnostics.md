# Page: reference/diagnostics (Diagnostics)

Page type: `format`. Section order: `52`. Wave: `2`. Model tier: `opus`.
...
One searchable index of every message the toolchain and runtime can produce: pattern, stage, severity, cause, fix, and the page that explains it. The page a search engine lands on.

## Read first

1. `docs/site/ARCHITECTURE.md`, all of it. The rules below are copied from it and are binding.
2. Parent facts files, before opening any source: `_facts/authoring/html.md`, `_facts/authoring/css.md`, `_facts/authoring/text-and-fonts.md`, `_facts/authoring/images.md`, `_facts/authoring/dynamic-text.md`, `_facts/authoring/lists.md`, `_facts/authoring/focus-and-navigation.md`, `_facts/authoring/theming.md`, `_facts/authoring/crt-linter.md`, `_facts/authoring/vram-budget.md`, `_facts/cli/ps2ui-bake.md`, `_facts/cli/ps2ui-check.md`, `_facts/runtime/errors-and-constants.md`, `_facts/cli/ps2ui.md`, `_facts/cli/previewer.md`. Reuse their fact ids; do not restate a parent fact differently. If one is wrong, write a `## disputes` section and stop.
...
- the facts files of every page under 'Read first'
- packages/layout/src/*.js - grep `throw`, `new Error(`, `warn`
- packages/baker/ps2ui_bake/*.py - grep `error:`, `warning`, `raise`, `ProjectError`, `not ok`
- runtime/ps2ui.h - the error codes
...
1. Every message from the parents is present -> union the parents' error/warning fact rows; count
2. Nothing was missed -> grep the sources for throw/error/warn sites, count distinct message templates, compare with the table's row count, report the difference and add the missing rows with their source line
3. Each row has either a test name or a command from a session that produced it -> the verified-by column
...
Grouped by stage in this order: HTML parse · CSS · layout · lints · bake · check · project and CLI · previewer · runtime load · runtime calls. One table per group with columns message · severity · cause · fix · page. A one-line intro per group saying where the message appears (stderr, TAP, return code).
...
- every page under Read first - the page column
...
- `diagnostics.index` - row count per group and the grep delta

## Out of scope

- explanations -> the page column

## Done when

- [ ] Environment block ran clean.
- [ ] `_facts/reference/diagnostics.md` exists with every fact id listed under "Facts to emit", each with a status.
- [ ] Every command shown on the page was run in this session; every output block is pasted from that run.
- [ ] Every screenshot listed exists under `assets/reference/diagnostics/`, is registered in `assets/assets.json` with its command, and has alt text.
- [ ] Frontmatter present: `id: reference/diagnostics`, `title`, `description`, `section`, `order: 52`, `version: 0.6.0`, `sources` (every path opened).
