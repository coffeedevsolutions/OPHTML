# Page: reference/compatibility (Compatibility)

Page type: `format`. Section order: `53`. Wave: `2`. Model tier: `sonnet`.
...
The versions that ship together, the one-version rule, what the format pledge means for old runtimes and new blobs, prerelease semantics on both registries, and the platform matrix.

## Read first

1. `docs/site/ARCHITECTURE.md`, all of it. The rules below are copied from it and are binding.
2. Parent facts files, before opening any source: `_facts/reference/uib-format.md`, `_facts/runtime/integrating.md`, `_facts/cli/ps2ui.md`. Reuse their fact ids; do not restate a parent fact differently. If one is wrong, write a `## disputes` section and stop.
...
- tools/check-versions.py - the rules and the numbers it reads
- CHANGELOG.md - the format paragraphs per release
- docs/releasing.md 'What a prerelease actually protects' and 'Names, and what deliberately did not change'
- packages/baker/pyproject.toml, packages/layout/package.json, packages/baker/ps2ui_bake/__init__.py
- runtime/vendor/README.md - gsKit pin; .github/workflows/hw.yml - PS2SDK floats
- .github/workflows/registry.yml - ubuntu and macOS from-registry runs
...
Format skeleton reduced to: Versions table · Format compatibility (table: runtime version · blob version · result) · Registries · Platforms table · Names (product vs format, two sentences).
...
- reference/uib-format - the pledge
- getting-started/installation - platforms
- runtime/integrating - toolchain
...
- `compat.versions`
- `compat.platforms`
- `compat.format-matrix`

## Out of scope

- the release procedure -> project/internals

## Done when

- [ ] Environment block ran clean.
- [ ] `_facts/reference/compatibility.md` exists with every fact id listed under "Facts to emit", each with a status.
- [ ] Every command shown on the page was run in this session; every output block is pasted from that run.
- [ ] Every screenshot listed exists under `assets/reference/compatibility/`, is registered in `assets/assets.json` with its command, and has alt text.
- [ ] Frontmatter present: `id: reference/compatibility`, `title`, `description`, `section`, `order: 53`, `version: 0.6.0`, `sources` (every path opened).
