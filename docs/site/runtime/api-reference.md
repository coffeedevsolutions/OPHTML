...
|---|---|---|---|
| `size_t ps2ui_arena_size(const void *data, size_t size)` | bytes, or 0 when the header is unreadable, the magic, version or alignment is wrong, the tables do not fit in `size`, the blob has no screens, or the arena would need more than `size_t` holds | blob | Reads counts only. Checks no CRC, reference or record, and does not touch the GS. |
...
Slot lookup walks every slot in the blob, at [ps2ui.c](repo:runtime/ps2ui.c#L1437). Two screens cannot share a slot name and stay distinguishable. Focus and visibility lookups walk only the current screen's range, at [ps2ui.c](repo:runtime/ps2ui.c#L1597).
...
The row-name convention is `prefix` followed by the decimal row index, built at [ps2ui.c](repo:runtime/ps2ui.c#L1686). Row focus names come from `data-repeat`, described on [Lists](page:authoring/lists#runtime-window).
