...
| `PS2UI_ERR_SIZE` (-11) | error | `ps2ui_tex_set` `len` differs from the reservation, or `ps2ui_clut_set` `ncolors` is above the baked count | pass the payload figure the bake printed | [VRAM budget](page:authoring/vram-budget#which-number-texset-wants) |
