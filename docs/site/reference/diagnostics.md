...
| `ps2ui-check: <path>: the <table> table (<n> entries at offset <off>) runs past the end of the <size>-byte file` | error, exit 2 | a header count or offset puts a table beyond the file; `ps2ui_load` refuses the same blob with `PS2UI_ERR_TRUNCATED` | rebake | [Errors and constants](page:runtime/errors-and-constants#load-check-order) |
| `ps2ui-check: <path>: the <table> table is at offset <off>, not a multiple of 4; the runtime refuses it (PS2UI_ERR_ALIGN)` | error, exit 2 | a table the runtime would read at an address a 32-bit load faults on | rebake | [Errors and constants](page:runtime/errors-and-constants#load-check-order) |
| `ps2ui-check: <path>: font <i>'s <glyph\|kern> table (<n> entries at blob offset <off>) runs past the <size>-byte blob`, or `... is at blob offset <off>, not a multiple of 4; the runtime refuses it (PS2UI_ERR_ALIGN)` | error, exit 2 | a font's glyph or kern array outside the blob, or where a 32-bit load faults | rebake | [Errors and constants](page:runtime/errors-and-constants#load-check-order) |
| `ps2ui-check: <path>: texture <i> has format <n>; the runtime reads only PSMCT32 and PSMT8` | error, exit 2 | a texture format the runtime refuses with `PS2UI_ERR_BOUNDS` | rebake | [Errors and constants](page:runtime/errors-and-constants#load-check-order) |
...
| `PS2UI_ERR_ALIGN` (-8) | error | the blob region address, a baked texture's `data_off`, or the arena is not 16-aligned; or the blob address, a table, or a font's glyph or kern table is not 4-aligned | align the buffer to 16 bytes; a misaligned table means the file was not written by this baker, so rebake it | [Integrating the runtime](page:runtime/integrating#limits-and-errors) |
