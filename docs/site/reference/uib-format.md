...
The runtime derives the pixel aspect from the last two fields and the canvas size, see [ps2ui_pixel_aspect_x1000](repo:runtime/ps2ui.h#L691). The C struct for every record on this page is listed on [C API reference](page:runtime/api-reference#structs).
