...
|---|---|---|---|---|
| serve.options | `ps2ui serve [project] [--uib BLOB] [--port PORT] [--screen NAME] [--theme THEME] [--no-watch] [--selftest]`. The table is under `## serve.options`. Parent fact `cli.ps2ui.serve.options` owns the list; this row adds the per-flag effects. | packages/baker/ps2ui_bake/ps2ui.py:499-511; serve.py:726-739 | `ps2ui serve --help` pasted on the page. README.md:591 calls its shorter list "the whole option list" and omits `--uib` (D5) | contradicts-readme |
