...
|---|---|---|---|---|
| cli.ps2ui.version | `ps2ui --version` prints `ps2ui <ps2ui_bake.__version__>` on stdout and exits 0. The released version prints `ps2ui 0.9.0`; this tree prints `ps2ui 0.10.0.dev0`. | packages/baker/ps2ui_bake/ps2ui.py:486-487; packages/baker/ps2ui_bake/__init__.py:39 | `ps2ui --version` printed `ps2ui 0.10.0.dev0`, exit 0; `TestConsoleScriptVersions` in `packages/baker/tests/test_baker.py:3690` asserts the string against `__version__` | verified |
...
| cli.ps2ui.serve.no-standalone | There is no `ps2ui-serve` command. `serve.py` declares `prog="ps2ui-serve"` in its own `main`, which no entry point reaches. | packages/baker/ps2ui_bake/serve.py:956-961; packages/baker/pyproject.toml:21-24 | `pyproject.toml` lists four console scripts and `ps2ui-serve` is not among them; `which ps2ui-serve` found nothing (D10) | verified |
