...
| cli.ps2ui.serve.no-standalone | There is no `ps2ui-serve` command. `serve.py` declares `prog="ps2ui-serve"` in its own `main`, which no entry point reaches. | packages/baker/ps2ui_bake/serve.py:868-873; packages/baker/pyproject.toml:21-24 | `pyproject.toml` lists four console scripts and `ps2ui-serve` is not among them; `which ps2ui-serve` found nothing (D10) | verified |
