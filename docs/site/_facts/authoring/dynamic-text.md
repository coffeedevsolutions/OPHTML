...
| slot.rules | A `data-slot` element must name the slot, hold exactly one text node with no child elements, have a placeholder that fits on one line, and use a name no other slot in the blob uses | packages/layout/src/box.js:202-222; packages/layout/src/paint.js:195-202, :329-335; packages/baker/ps2ui_bake/quads.py:591-611 | the four errors triggered this session, each exit 1, see the four rows below | verified |
