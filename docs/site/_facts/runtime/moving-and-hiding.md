...
| visible.bits | The bitmap is carved from the arena and sized from `n_focus`, so every node a blob declares can be hidden. There is no fixed ceiling | runtime/ps2ui.h:710-715; runtime/ps2ui.c:939-941, runtime/ps2ui.c:1642-1645 | ok 167 `every focus node on this screen can be hidden and shown` | verified |
...
| offset.queries | Every geometry query stays in UI coordinates, the coordinates the blob was authored in. An app drawing its own art beside the UI adds the offset itself, reading it back with `ps2ui_offset_get` | runtime/ps2ui.h:748-757; runtime/ps2ui.c:1627-1631 | ok 408; `<scratch>/probe` read `ctx.focus_nodes[ctx.focus]` before and after `offset_set(80, -60)` and printed `x=464 y=209 w=128 h=129` both times | verified |
