...
| serve.parity | `ps2ui serve --selftest` asserts the served frame is byte-identical to what `--preview` writes, which is what keeps the browser page and the baker one renderer | packages/baker/ps2ui_bake/serve.py:797-814 | `ps2ui serve examples/memcard --selftest` printed `ok - the frame is byte-identical to --preview` and `PASS: 6 route(s)` | verified |
