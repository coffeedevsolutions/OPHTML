...
| offset.new | The draw-time offset is the 0.6.0 addition (F27). Runtime visibility shipped in 0.3.0 | CHANGELOG.md:1238-1245 (0.6.0 Added); CHANGELOG.md:1772-1774 (0.3.0 Added) | `grep -n '^## ' CHANGELOG.md` puts line 1073 under `## 0.3.0` and line 539 under `## 0.6.0`. The previous version of this row said 541 and 35, which F41(a) showed had drifted into the 0.6.0 offset entry and the open section's prose as the file grew from the top | verified |
...
`README.md:438-441` says `ps2ui serve` and `ps2ui-bake --preview` "apply the same offset
