...
| compat.versions | The tree carries five version numbers: `ophtml` (PyPI) 0.6.0.dev0, `@ophtml/layout` (npm) 0.6.0-dev.0, the ui.json IR 1, the `.uib` format 7, and the runtime macro `PS2UI_VERSION` 7. `check-versions.py` holds all five to each other. | reference/compatibility | restated from `_facts/reference/compatibility.md`, row `compat.versions`, verified there by `python3 tools/check-versions.py --except-tag` | verified |
