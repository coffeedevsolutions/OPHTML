...
- `grep -n '^- \*\*\|^### ' CHANGELOG.md` located every bullet and
  ...
  same output counted 12 entries under the open section's three headings
  against 12 bullets under the page's three.

| id | fact | source | verified by | status |
|---|---|---|---|---|
| changelog.mapping | The open 0.10.0.dev0 section maps **twelve bullets for twelve entries**, the console, its theme check and preview, the launcher's download and page, the released-section check, the loader fuzzing and its two fixes, the registry.yml cleanup and S3's four caps entries: `### Added` at 17 with six, `### Changed` at 149 with one and `### Fixed` at 166 with five in `CHANGELOG.md`, and `### Added` at 21 with six, `### Changed` at 48 with one and `### Fixed` at 56 with five on the page, and rule 14 holds the page's `## 0.10.0.dev0` to that count as entries land. **Rule 15 reads this cell**, which is what makes the arithmetic a claim something checks rather than prose that goes stale at the next entry: two versions of this row were corrected by hand twice in one session before that rule existed, each time after the cell beside them had just been corrected. The 0.9.0 bullet below it maps to the one entry in `CHANGELOG.md`'s 0.9.0 section, F47's | `CHANGELOG.md:17, 19, 41, 63, 111, 127, 137` (the open section's `### Added`, the console's entry, the theme-check entry, the caps, the launcher's download, the released-section check and the loader fuzzing); `CHANGELOG.md:149, 151` (its `### Changed` and the registry.yml entry); `CHANGELOG.md:166, 168, 181, 192, 202, 211` (its `### Fixed`, the two faults the fuzzing found and the three the caps closed); `CHANGELOG.md:238, 240` (0.9.0's `### Changed` and F47's entry). The counts in this row are held to rule 15 of `check-versions.py`, which derives them from the same pass rule 14 counts with | `python3 tools/check-versions.py` printed "the Changelog page restates the open 0.10.0.dev0 section one for one: 12 bullet(s) for 12 entries" and "the changelog.mapping row names the 7 number(s) rule 14 counts" | verified |
| changelog.earlier-releases | The Earlier releases table's six rows read straight from the CHANGELOG headings and the entries their headlines restate, every one of them format v7: `## 0.8.0 — 2026-09-23` (the Windows and Intel Mac CI arms, and the version banners held to what commands print); `## 0.7.0 — 2026-09-17` (`vendor-runtime --starter`, the `:focus` rules, the eight checked keywords); `## 0.6.0 — 2026-09-12` (`ps2ui_offset_set`, and `ps2ui check` given `ps2ui build`'s settings); `## 0.5.0 — 2026-09-06`; `## 0.4.0 — 2026-09-06` (the v7 stability pledge and vendored DejaVu fonts); `## 0.3.0 — 2026-09-04` (four moves since 0.2.0, v4 through v7; first tagged release). **0.6.0 and 0.7.0 were missing from this table** from their own releases until the 0.9.0 cut added them with 0.8.0 | `CHANGELOG.md:289, 305, 335` (0.8.0, the version-banner entry, the platforms entry); `CHANGELOG.md:1249, 1265, 1325, 1370` (0.7.0 and its three restated entries); `CHANGELOG.md:1520, 1524, 1622` (0.6.0 and its two); `CHANGELOG.md:1712, 1762, 1852` (the three older headings) | read against `grep -n '^## ' CHANGELOG.md` and each cited entry at the 0.9.0 cut | verified |
| compat.versions | This tree's `ophtml` is 0.10.0.dev0 and `@ophtml/layout` is 0.10.0-dev.0, the same version in two spellings; the release both registries serve is 0.9.0, tagged `v0.9.0`. | reference/compatibility (parent) | after step 9 `check-versions.py --except-tag` printed "@ophtml/layout 0.10.0-dev.0 and ophtml 0.10.0.dev0 are the same version in the two spellings" | verified |
| compat.format-matrix | Format v7 is unchanged since 0.9.0: CHANGELOG.md counts zero format moves since that release, and a v7 blob loads under a v7 runtime regardless of the two products' package versions. | reference/compatibility (parent) | after step 9 `check-versions.py` printed "CHANGELOG counts zero format moves since 0.9.0, and v7 -> v7 is 0" | verified |
