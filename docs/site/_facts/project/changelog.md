...
- `python3 tools/check-versions.py --except-tag` printed 30 `ok -`
  lines and 1 `skip -` line, exit 0. **This line said 27 until now**,
  a count from an earlier session that three added rules had passed;
  each of the 7 lines below was re-matched against this run's output. The 7 lines pasted on the page
