...
|---|---|---|---|---|
| changelog.mapping | The open 0.10.0.dev0 section maps **one bullet for one entry**, the registry.yml cleanup: `### Changed` at 17 with one in `CHANGELOG.md` and `### Changed` at 21 with one on the page, and rule 14 holds the page's `## 0.10.0.dev0` to that count as entries land. The 0.9.0 bullet below it maps to the one entry in `CHANGELOG.md`'s 0.9.0 section, F47's | `CHANGELOG.md:17, 19` (the open section's `### Changed` and its entry); `CHANGELOG.md:48, 50` (0.9.0's `### Changed` and F47's entry). The counts in this row are held to rule 15 of `check-versions.py`, which derives them from the same pass rule 14 counts with | `python3 tools/check-versions.py` printed "the Changelog page restates the open 0.10.0.dev0 section one for one: 1 bullet(s) for 1 entry" and "the changelog.mapping row names the 3 number(s) rule 14 counts" | verified |
| changelog.earlier-releases | The Earlier releases table's six rows read straight from the CHANGELOG headings and the entries their headlines restate, every one of them format v7: `## 0.8.0 — 2026-09-23` (the Windows and Intel Mac CI arms, and the version banners held to what commands print); `## 0.7.0 — 2026-09-17` (`vendor-runtime --starter`, the `:focus` rules, the eight checked keywords); `## 0.6.0 — 2026-09-12` (`ps2ui_offset_set`, and `ps2ui check` given `ps2ui build`'s settings); `## 0.5.0 — 2026-09-06`; `## 0.4.0 — 2026-09-06` (the v7 stability pledge and vendored DejaVu fonts); `## 0.3.0 — 2026-09-04` (four moves since 0.2.0, v4 through v7; first tagged release). **0.6.0 and 0.7.0 were missing from this table** from their own releases until the 0.9.0 cut added them with 0.8.0 | `CHANGELOG.md:99, 115, 145` (0.8.0, the version-banner entry, the platforms entry); `CHANGELOG.md:1059, 1075, 1135, 1180` (0.7.0 and its three restated entries); `CHANGELOG.md:1330, 1334, 1432` (0.6.0 and its two); `CHANGELOG.md:1522, 1572, 1662` (the three older headings) | read against `grep -n '^## ' CHANGELOG.md` and each cited entry at the 0.9.0 cut | verified |
