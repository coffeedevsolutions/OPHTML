...
- `grep -n '^- \*\*\|^### ' CHANGELOG.md` located every bullet and
  ...
  same output counted 33 entries under the open section's two headings
  against 33 bullets under the page's three.

| id | fact | source | verified by | status |
|---|---|---|---|---|
| changelog.mapping | Every 0.8.0.dev0 bullet on this page maps to one entry in `CHANGELOG.md`'s open section, and the map is now onto: **thirty-three bullets for thirty-three entries**, one each. The file carries two headings, `### Added` with four entries and `### Fixed` with twenty-nine. The page carries three, because the twenty-nine split by who is affected rather than by kind: nineteen changed the toolchain a project uses, ten changed the checker that reads these pages and no compiler, runtime or format behaviour. **The page restated five of the twenty-four then open until this change** -- its own sentence claimed only that every bullet on it had an entry in the file, which held, so nothing failed while nineteen were missing. Found by F41(a) while re-pointing these citations, widened by F42, filed rather than fixed both times because those changes were about the checker | `CHANGELOG.md:17, 19-92, 143, 145` (`### Added`, its four entries, `### Fixed`, the first of its twenty-nine). **Written as a comma list on purpose.** `FACTS_CITE` pins every member of one and `--fix` moves each. The shape this cell used to use wrote every member after the first as a colon and a number with no path in front of it, which named nothing the checker could read: it pointed at 32 for `### Fixed` while that heading sat at 49, and at 34, 56, 71 and 90 for four bullets that were at 51, 73, 88 and 107. **The numbers in that last sentence are spelled without their colons on purpose**, because F42 made the shape a failure and nothing can tell an example of it from a use of it | re-derived this session: a pass over the open section counts `### Added` at 17 with four entries from 19 and `### Fixed` at 143 with twenty-nine from 145, thirty-three in all; the same pass over the page counts `### Added` at 23 with four, `### Fixed in the toolchain` at 50 with nineteen and `### Fixed in the documentation checker` at 140 with ten, thirty-three in all. **The previous version of this row, including the one F41(a) itself wrote, claimed an `awk` run that put the headings at 17 and 32** -- 32 is the second Added entry, and no run of that command ever said otherwise | verified |
| changelog.earlier-releases | The Earlier releases table's three rows read straight from the CHANGELOG headings and their format paragraphs: `## 0.5.0 — 2026-09-06` (format v7, zero moves since 0.4.0); `## 0.4.0 — 2026-09-06` (format v7, zero moves since 0.3.0, the v7 stability pledge and vendored DejaVu fonts added in that section); `## 0.3.0 — 2026-09-04` (format v7, four moves since 0.2.0: v4 display aspect, v5 kerning, v6 texture kinds, v7 tint table; first tagged release). | `CHANGELOG.md:1359, 1362, 1397` (the 0.5.0 heading, the `vendor-runtime` entry its headline restates, and the format paragraph that section carries); `CHANGELOG.md:1409, 1411, 1424, 1435` (0.4.0, its format paragraph, the stability-pledge entry, the vendored-fonts bullet); `CHANGELOG.md:1499, 1501, 1513` (0.3.0, its format paragraph naming the four moves, the first-tagged sentence). **The previous version of this cell cited 0.7.0, 0.6.0 and 0.5.0 for a table whose three rows are 0.5.0, 0.4.0 and 0.3.0**, and two of its three numbers sat on prose in the middle of a release rather than on a heading. It stayed green throughout: whatever those lines once named, a `--pin` run recorded the text they held by then, and the drift test has faithfully kept them on it since. **A pin proves a line has not moved, not that it was the right line.** | a pass over `^## ` in this session found the three release headings and their format paragraphs, and every line in the source cell was read directly before it was pinned. **THE TEN NUMBERS USED TO BE RESTATED HERE AND ARE GONE ON PURPOSE.** They were an unpinned second copy of the ten in the source cell, so each edit to `CHANGELOG.md` rotted them while the pinned ten were relocated and stayed right. That happened twice in this session, the second time inside the change that had just corrected them. `BACKLOG.md`'s closed-log marker settled this shape already: keep what the run established, drop the figure the next commit invalidates. The source cell is the copy something checks. The table's three headlines were re-read against those sections: `vendor-runtime` ships the runtime from the installed package, the v7 pledge and `fonts/vendor/` are both in the 0.4.0 section, and 0.3.0 is the first tagged release, with four moves v4 through v7 | verified |
