...
|---|---|---|---|---|
| changelog.mapping | The 0.9.0 bullet on this page maps to the one entry in `CHANGELOG.md`'s 0.9.0 section, F47's: **one bullet for one entry**, `### Changed` at 17 with one in the file and `### Changed` at 22 with one on the page. The 0.8.0 section left this page at this cut for a row in the Earlier releases table, as every release before it did; its thirty-two entries are in the file | `CHANGELOG.md:17, 19` (`### Changed` and F47's entry). The counts in this row are held to rule 15 of `check-versions.py`, which derives them from the same pass rule 14 counts with | `python3 tools/check-versions.py --except-tag` at the 0.9.0 cut printed "the Changelog page restates the open 0.9.0 section one for one: 1 bullet(s) for 1 entry" and "the changelog.mapping row names the 3 number(s) rule 14 counts" | verified |
| changelog.earlier-releases | The Earlier releases table's six rows read straight from the CHANGELOG headings and the entries their headlines restate, every one of them format v7: `## 0.8.0 — 2026-09-23` (the Windows and Intel Mac CI arms, and the version banners held to what commands print); `## 0.7.0 — 2026-09-17` (`vendor-runtime --starter`, the `:focus` rules, the eight checked keywords); `## 0.6.0 — 2026-09-12` (`ps2ui_offset_set`, and `ps2ui check` given `ps2ui build`'s settings); `## 0.5.0 — 2026-09-06`; `## 0.4.0 — 2026-09-06` (the v7 stability pledge and vendored DejaVu fonts); `## 0.3.0 — 2026-09-04` (four moves since 0.2.0, v4 through v7; first tagged release). **0.6.0 and 0.7.0 were missing from this table** from their own releases until the 0.9.0 cut added them with 0.8.0 | `CHANGELOG.md:68, 84, 114` (0.8.0, the version-banner entry, the platforms entry); `CHANGELOG.md:1028, 1044, 1104, 1149` (0.7.0 and its three restated entries); `CHANGELOG.md:1299, 1303, 1401` (0.6.0 and its two); `CHANGELOG.md:1491, 1541, 1631` (the three older headings) | read against `grep -n '^## ' CHANGELOG.md` and each cited entry at the 0.9.0 cut | verified |
| compat.versions | `ophtml` is 0.9.0 and `@ophtml/layout` is 0.9.0, the same version in two spellings, tagged `v0.9.0` once step 7 runs. | reference/compatibility (parent) | the 0.9.0 cut's `check-versions.py --except-tag` printed "@ophtml/layout 0.9.0 and ophtml 0.9.0 are the same version in the two spellings" | verified |
| compat.format-matrix | Format v7 is unchanged since 0.8.0: CHANGELOG.md counts zero format moves since that release, and a v7 blob loads under a v7 runtime regardless of the two products' package versions. | reference/compatibility (parent) | the 0.9.0 cut's `check-versions.py` printed "CHANGELOG counts zero format moves since 0.8.0, and v7 -> v7 is 0" | verified |
