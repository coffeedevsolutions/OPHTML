...
|---|---|---|---|---|
| changelog.mapping | The open 0.10.0.dev0 section is empty on both sides, **zero bullets for zero entries**, and rule 14 holds the page's `## 0.10.0.dev0` to that count as entries land. The 0.9.0 bullet below it maps to the one entry in `CHANGELOG.md`'s 0.9.0 section, F47's, under `### Changed` in the file and on the page | `CHANGELOG.md:31, 33` (0.9.0's `### Changed` and F47's entry). The counts in this row are held to rule 15 of `check-versions.py`, which derives them from the same pass rule 14 counts with | `python3 tools/check-versions.py` after step 9 printed "the Changelog page restates the open 0.10.0.dev0 section one for one: 0 bullet(s) for 0 entries" and "the changelog.mapping row names the 1 number(s) rule 14 counts" | verified |
| changelog.earlier-releases | The Earlier releases table's six rows read straight from the CHANGELOG headings and the entries their headlines restate, every one of them format v7: `## 0.8.0 — 2026-09-23` (the Windows and Intel Mac CI arms, and the version banners held to what commands print); `## 0.7.0 — 2026-09-17` (`vendor-runtime --starter`, the `:focus` rules, the eight checked keywords); `## 0.6.0 — 2026-09-12` (`ps2ui_offset_set`, and `ps2ui check` given `ps2ui build`'s settings); `## 0.5.0 — 2026-09-06`; `## 0.4.0 — 2026-09-06` (the v7 stability pledge and vendored DejaVu fonts); `## 0.3.0 — 2026-09-04` (four moves since 0.2.0, v4 through v7; first tagged release). **0.6.0 and 0.7.0 were missing from this table** from their own releases until the 0.9.0 cut added them with 0.8.0 | `CHANGELOG.md:82, 98, 128` (0.8.0, the version-banner entry, the platforms entry); `CHANGELOG.md:1042, 1058, 1118, 1163` (0.7.0 and its three restated entries); `CHANGELOG.md:1313, 1317, 1415` (0.6.0 and its two); `CHANGELOG.md:1505, 1555, 1645` (the three older headings) | read against `grep -n '^## ' CHANGELOG.md` and each cited entry at the 0.9.0 cut | verified |
