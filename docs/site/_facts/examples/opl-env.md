...
| example.opl-env.ci-step | CI builds and checks this blob in the step named "OPL environment end to end", which runs `./examples/opl-env/build.sh` and nothing else; a later, separate step named "Committed screenshots match the renderer" re-runs `git diff --exit-code` over all three examples' `screenshots/` directories together, and a further step "Example figures match their blobs" runs `tools/check-example-figures.py` | .github/workflows/ci.yml:211-212 (build step), 259-260 (figures step), 329-346 (screenshot-diff step) | read | code-only (the workflow itself was not executed in this session, only the same shell commands it runs, each of which was run directly) |
