...
| example.memcard.ci-step | CI builds this blob in the step named "Example builds end to end (includes runtime tests)", which runs `./examples/memcard/build.sh` and nothing else | .github/workflows/ci.yml:241-242 | read | code-only (the workflow itself was not executed in this session, only the same shell command it runs) |
