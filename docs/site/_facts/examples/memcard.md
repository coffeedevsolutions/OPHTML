...
| example.memcard.trailer | `ps2ui-check examples/memcard/build/ui.uib` passes 63 checks with the trailer `640x448 at 4:3, 2 screen(s), 1062 commands, 11 textures, 6 slots` and the arena note `1662 bytes on the EE (1750 on a 64-bit host ...)` | packages/baker/ps2ui_bake/check.py (parent `check.tap-shape`, `check.arena-note`) | `ps2ui-check examples/memcard/build/ui.uib`, output pasted on the page, exit 0 | verified |
| example.memcard.vram | The blob uses 160 KiB of a 736 KiB default budget (21%) | packages/baker/ps2ui_bake/vram.py (parent `check.vram.default`) | `ps2ui-check examples/memcard/build/ui.uib` printed `ok 60 - VRAM 160 KiB within budget 736 KiB`; a direct `ps2ui-bake` run in this session printed `textures 163840 B of 753664 B budget (21%)` for the same blob | verified |
