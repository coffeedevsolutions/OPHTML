...
| example.memcard.runtime-tests | `make -C runtime test` runs `test_runtime` (410 checks, this blob plus three generated fixtures) then `test-narrow` (5 checks, this blob plus the huge-arena fixture), in that order, so the narrow suite's `PASS: 5 checks` line prints after the main suite's `PASS: 410 checks` line despite the target list naming `test-narrow` last as a prerequisite | runtime/Makefile:6 (`UIB ?= ../examples/memcard/build/ui.uib`), runtime/Makefile:23-24, runtime/Makefile:246-247 | `make -C runtime test` in this session: `PASS: 5 checks, 0 failure(s)` at output line 49, `PASS: 410 checks, 0 failure(s)` at line 464, both exit 0 | verified |
