...
| F12 | **Packaging.** Publish `@ophtml/layout` to npm and `ps2ui-bake` to PyPI, plus one `ps2ui` wrapper CLI (`build`, `dev`, `fontgen`) so the quick start is two installs and one command instead of PYTHONPATH incantations. | 8 | 1 | 0.8 | 2 | **3.2** |
