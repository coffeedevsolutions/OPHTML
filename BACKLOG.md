...
| F29 | **The audit that exists to catch stale ticks has now missed rows twice, and the second miss was mechanical rather than a judgement call.** The 2026-09-07 pass ticked 17 rows and carries its own section on how it went wrong — it searched the vocabulary of the PROBLEM rather than the FIX, and reported B3 and F3 wrongly before correcting. It then left B8 and F24 shipped-but-unticked anyway, and a fresh sweep on 2026-09-12 missed B8 a second time. **The cause is not carelessness, it is that the board is not machine-readable and nothing asserts that it is.** B8's row read `| B8* |`, a marker no legend in this file defines, which no ID-keyed regex matches; the Sprint 1 status line said `✅ B9 + B8` while the row said neither. F28 records the same shape one layer out (an inventory built by grepping the mechanism's vocabulary could not match prose naming the field), and `docs/method.md` records it a third time. Four instances in a week is a fence's worth. **WHAT A FENCE COULD ACTUALLY ASSERT, none of which needs to read prose:** every row's ID matches `(F|B|S|P)[0-9]+` exactly, with no trailing punctuation and no third marker state — two states, ✅ or nothing; every ID is unique and appears in exactly one table (two ticked F25s once made `grep` ambiguous, which F26's row records); and no ID is called shipped by a status line while its row is unticked, which is the check that would have caught B8 on the day. That is `grep` over table syntax, not claim-matching over prose — the thing rule 6's note warns off — and it is the same kind of structural check `tools/check-findings.py` already runs over `findings.yaml`. **What it would NOT catch**, and the row should not pretend otherwise: a row whose work shipped and whose ID and marker are both well-formed. F24 was exactly that, and only reading the code found it. So the fence bounds the problem rather than solving it, and the residue is the same one F28 leaves open. **THE STATUS-LINE CHECK WAS RUN ONCE BY HAND WHILE WRITING THIS ROW AND IMMEDIATELY FOUND A THIRD.** B2's row was unticked and audited as *unsettled*, while the Sprint 2 status line has said `✅ B2 baseline seam fixed` since 2026-08-17; the fix is in `atlas.py:75,130-132` and fenced at `test_baker.py:397`, both citing the row by name. So the cheapest of the three proposed checks has a hit rate of one row per run on a board audited five days earlier, which is the argument for building it. **Filed 2026-09-12** after B8, F24 and B2 were ticked; the trigger was a stranger-path report checking the board against the code rather than against itself. |
...
**THE 2026-09-07 PASS BELOW MISSED TWO ROWS, AND THE AMENDMENT IS WORTH
MORE THAN THE PASS.** B8 and F24 had shipped and stayed unticked; a
fresh sweep on 2026-09-12 missed B8 a second time. Neither was a
judgement call:
