...
| F35 | **`--starter` is the answer to the console half and no page a stranger reads mentions it.** `ps2ui vendor-runtime --starter` writes a `main.c` and a `Makefile` that build to an ELF as they stand, which is precisely the no-clone console path Phase 4's exit gate is about. It is documented in `docs/deploying.md` and appears on **zero** pages of the documentation site; the live `runtime/integrating/` page returns no match for the string and its table states that `vendor-runtime` takes *"one positional and one flag"*, listing only `dest` and `--force`. Only the tool's own stdout advertises it. Measured 2026-09-17 against the published site. The consequence is that the site's console path is built on `make -C runtime/sample`, `tools/make_testcard.py`, `examples/channel6/build/ui.uib` and `docs/bringup.md` — five things a reader who did not clone cannot run — while the one command that solves their problem is invisible. **The authoring half is clone-free and the console half is not, on the site**, which is the same shape as the contradiction F31 fixed one layer down. |
| F36 | **`pip install ophtml` fails on a current macOS or Debian box and the Installation page does not say so.** A cold run got `error: externally-managed-environment` (PEP 668) on the first command the page gives. The error names the fix, so a motivated reader recovers in about twenty seconds, but the page shows `$ which ps2ui` resolving to `/usr/local/bin/ps2ui`, which implies a global install that Homebrew Python refuses. Nothing on the page mentions `venv`, `pipx` or PEP 668. **The page has no "where to get a TTF" line either**, and the quickstart's first command is `ps2ui fontgen "$TTF_REGULAR" "$TTF_BOLD"` with both variables undefined: a stock macOS install has almost no plain `.ttf` (`/System/Library/Fonts` is mostly `.ttc`), and the cold run only got the document's exact numbers because that machine happened to carry `/Library/Fonts/DejaVuSans.ttf`. Two short paragraphs remove the first and third obstacles a newcomer meets. |
