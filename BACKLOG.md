...
| F5 | **Precompiled GIF/DMA chains.** The roadmap's performance endgame: bake per-state GIF packets so a frame is a DMA kick instead of per-quad gsKit calls. Near-zero CPU per frame, but the biggest hardware-knowledge item in the backlog and pointless to attempt before F1 exists to validate it. **Sequencing against F26:** a chain pays off because the geometry is fixed, so the animated share of a screen is a direct discount on what this can buy, and the chain's design has to decide up front how much stays patchable. If F26 is wanted, its scope belongs on the table before this is built rather than after. P3d's content sweep prices both: the per-command term is F5's ceiling, and the same fit says what an animated frame costs, since animating means re-walking rather than reusing. | 7 | 2 | 0.5 | 6 | **1.2** |
...
| F26 | **Runtime geometry — animation and transitions.** Geometry is baked, and the complete runtime surface is visibility, slot text, streamed textures, CLUT/theme, screen, focus and list windowing: nothing moves, scales or fades a node, so a cascade, a coverflow, a slide transition or a focus ring that travels cannot be expressed at all — an app can only cut between baked states. The shells this project is measured against are animated (PSBBN is the reference for a polished PS2 launcher), so this is the polish gap rather than a capability gap: everything ps2ui draws is already correct, it just arrives instantly. Three tiers, cheapest first. **(a) Per-node offset**: `ps2ui_offset_set(ctx, "name", dx, dy)` applied to a subtree at render time — one add per command, a per-focus-node offset table shaped like the visibility bits, no format change of consequence. Buys slide-in panels, travelling focus rings, parallax, and on its own would score ≈6. **(b) Per-node scale and opacity**: opacity multiplies into the tint alpha the GS already applies and the tint table has been per-command since P3b, so it is nearly free; scale recomputes quad corners and is where P1k's half-texel rule needs re-deriving rather than reused. **(c) Declarative timelines**: a `@keyframes` subset baked into the blob so an app plays a named transition instead of driving a value per frame — the tier that makes this feel authored in CSS rather than in C. Wants F8 (`position: absolute`) first for anything that overlaps without nesting; `lint.js` already carries forward cover for exactly that. **Pulls against F5**, see the note under it. | 6 | 2 | 0.5 | 6 | **1.0** |
...
**Explicitly post-MVP:** F26 runtime geometry. Wanted, not needed to
ship — nothing in the stated use cases fails without it, and everything
that does need it (F8, and F5's scope) is upstream. Tier (a), the
per-node offset, is the part worth pulling forward if a shell asks for
motion before the rest is ready.
