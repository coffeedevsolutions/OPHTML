...
| B17 | **`ps2ui check <blob.uib>` dies with an unhandled `UnicodeDecodeError`.** `check` takes a PROJECT — a `ps2ui.json` — and the blob validator is the separate `ps2ui-check`. Handing it a `.uib` reaches a text read of a binary file and ends in `UnicodeDecodeError: 'utf-8' codec can't decode byte 0x80 in position 8`, a Python traceback with no diagnosis. Reproduced on `801d1ad`. It is a plausible thing to try: the two commands are one hyphen apart, the tutorial teaches `ps2ui check` with no argument, and `--help` says only `usage: ps2ui check [-h] [project]`. A file that does not parse as a project should say so and name `ps2ui-check` for a blob; the traceback is the only output in the toolchain that tells a reader nothing at all. |
| B18 | **`ps2ui serve --port <busy>` prints a traceback, and `--help` promises the opposite.** `serve.py:655`'s comment is explicit and the behaviour is deliberate — *"an EXPLICIT --port fails hard, because a person who named a port meant that port"* — and the default genuinely does wander, 8080 to 8081, which the quickstart describes correctly. But the help text on the flag reads `default 8080, moving up when it is busy`, attached to `--port`, so it reads as a promise the flag does not keep; and the deliberate refusal surfaces as a raw `OSError: [Errno 48] Address already in use` from `ThreadingHTTPServer` rather than a message. Two small fixes, neither of them the behaviour: word the help so the wandering is the DEFAULT's property rather than the flag's, and catch the bind error to say which port was busy and that an explicit one is not moved. |
| B19 | **A `## 0.6.0` section on the changelog page states the 0.7.0 versions.** `docs/site/project/changelog.md:17-19` heads a section `## 0.6.0` and then reads `` `ophtml` 0.7.0, `@ophtml/layout` 0.7.0. `` Present on `main` before the 0.8.0.dev0 back-to-development and not caused by it — found while checking that branch's own edits and deliberately left out of it, since folding an unrelated fix into a version bump is how a version bump stops being reviewable. Either the heading or the pair is wrong and the page cannot say which, which is the point: **no check reads this**. `check-doc-versions.py` (`0932011`) reads the version BANNERS a command prints and this is neither — it is a version stated in prose under a heading, which that tool's docstring names as the case it deliberately does not cover. |
...
| F38 | **`border-radius` costs 3x the records and nothing says so.** Measured on `801d1ad`: one flex box with a background and a text span bakes to **3 records** square and **11 records** with `border-radius: 8px`. The cold evaluation's own UI went from roughly 24 records to **354** after rounding its corners, with no warning at build time and no note in the CSS reference, which is otherwise exhaustive and accurate about every property it lists. This is not an argument for refusing rounded corners — it is the ordinary thing a console UI wants — but the record count is the number `ps2ui check` budgets against and the one a data-heavy screen runs out of, so a reader should be able to find the price before they pay it. A line in the CSS reference beside `border-radius`, or a lint that reports the multiplier when a rounded box is repeated, would do. |
