...
| F30 | **Nothing maps a code change to the documentation it invalidates, and the graph to do it already exists unread.** The `docs/site/` library carries the dependency data as a side effect of how each page is produced: every page's frontmatter has `sources: [...]`, the repo files it was verified against, and every `_facts/<id>.md` row carries a `source` column naming `file:line`. Measured on `claude/sleepy-feynman-kzrs87` at 20 pages: **653 fact rows over 94 distinct repo sources**, with `README.md` cited by 13 pages and `runtime/ps2ui.h` by 11. Nothing reads any of it, so the question *"I changed `ps2ui.h`, which pages are now suspect?"* has no answer short of re-reading 46 pages. **THE PRECEDENT IS ALREADY IN THE TREE AND THE TOOL SHOULD COPY IT.** `tools/check-findings.py --impact F-023` answers *"what breaks if this dies"* over the findings ledger; this is the same query over a different graph, so it wants the same command grammar rather than a new one. Three modes: `--impact <path>` lists the pages naming that file in `sources`, then the pages downstream of those through the facts they consume; `--changed` reads a diff and names pages whose sources moved while the page did not, as a **warning**, because a change can be genuinely doc-neutral and a fence that fires on correct work gets an exemption bolted on within a week; `--orphans` runs it backwards, finding `sources` entries and fact `file:line` references that no longer resolve, which is the direction that rots silently. **DERIVE THE GRAPH, NEVER STORE IT.** A hand-maintained `deps.yaml` is a second copy that goes stale, which is the defect this item exists to catch — the same reasoning as F26's build hook and B8's breakdown. **TWO IMPLEMENTATION CONSTRAINTS FOUND BY TRYING, NOT BY READING.** Fact cells contain `|` inside backticks, so a naive `split('|')` shreds rows — a first pass over these files produced 30-odd bogus statuses before the parser respected code spans. And `sources:` lists mix repo code with intra-doc paths like `docs/site/_facts/README.md`, so the tool has to separate the two or every doc edit flags every page. **WHAT IT CANNOT DO, and the row should not pretend otherwise:** it is file-level, not claim-level, so a one-line comment fix flags the same pages as an API removal; and it cannot see a page that *should* have cited a file and did not, which is F24's failure mode one layer up. It bounds the review surface rather than deciding correctness. **SEQUENCING: after the doc waves, not during.** With wave 0 alone the graph is nine nodes and the tool would be falsified against nothing. **Filed 2026-09-13**, prompted by the Tier 1 shipped-comment fixes (#129) raising how a repo change is traced to the docs it breaks. Worth knowing before anyone starts: the same branch already carries **21 `contradicts-readme` fact rows**, each with an id, a source and a verification — the README repair list, enumerated and evidenced by the page agents, and better than the drift table it would otherwise be reconstructed from. |
