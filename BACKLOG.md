...
| F28 | **Three documents still teach a mechanism that was deliberately removed, and no rule can see it.** `README.md:672` and `CONTRIBUTING.md:16` both instruct a contributor to run `make -C runtime test-compat`; `README.md:675` describes what it does (*"repeats everything with `PS2UI_GSKIT_HAS_FUNCTION=0` for older gsKit (text loses tinting)"*); and `docs/PLAN.md:75` states the runtime suite is *"run twice (modern gsKit and the `HAS_FUNCTION=0` fallback)"* as part of a phase's verification. **None of it exists.** Neither the Makefile target nor the macro is anywhere in the tree, so the README's own verification block ends in `make: *** No rule to make target 'test-compat'`. **This is not drift — it is a correct removal whose documentation was never followed through.** #39 took the whole mechanism out on purpose and said so in the commit message, enumerating `test-compat` among the things it removed, because [F-005](docs/findings.md) settled that gsKit has **no** `GSTEXTURE::Function` field and hardcodes `TEX0.TFX = 0` (MODULATE) at all 30 of its `TEX0` sites. The "older gsKit fallback" the three documents describe is a path gsKit has never had. **THE PART THAT IS NOT BOOKKEEPING:** `tools/check-findings.py` rules 6 and 8 exist for exactly this and cannot reach it. Both key on **citations**, and rule 8's own docstring says why — it *"cannot see prose that states one while citing nothing"* — and both are scoped to **overturned** findings, while F-005 is live and correct. So prose that contradicts a LIVE finding, citing nothing, is outside every rule in the checker. Whether that class can be fenced at all is the question this item has to answer, and a `--render`-style keyword sweep is the obvious wrong answer: an earlier design of the tool tried matching claim text against prose and was dropped as too fragile to survive a month (rule 6's own note). **The worst of the five sites is not a document.** `examples/channel6/ui/channel6.css:466` is a bench decision tree telling an operator that text brighter than its block means *"step 4, GSTEXTURE::Function, or an unintended `PS2UI_GSKIT_HAS_FUNCTION=0` build"* — two causes F-005 proves impossible, printed as the diagnosis for a symptom that must therefore have a third. That is an instrument pointing away from its own fault, in front of a person with a console. `docs/PLAN.md:232` needs a second look rather than a search-and-replace: it lists the fallback under *"Left: steps 3-9"*, which is stale on the bring-up count too. **Found 2026-09-09** by running the README's verification block during F27, which is the first time in this session anything ran it end to end. |
